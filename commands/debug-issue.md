...
This command is a thin entry point. The full playbook lives in the **`debug-error`** skill: a hypothesis, a failing check, then a fix. When the error is from production and Sentry is connected, fetch that context first. When the fix pattern is unfamiliar, use Firecrawl or Context7. Do not require Sequential Thinking — it is not in the default MCP template.
