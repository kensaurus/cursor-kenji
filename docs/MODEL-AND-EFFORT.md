...
Skill descriptions ride in every request: ~43k chars after this pass (~11k tokens) always on; `validate-skills` prints the total and fails above its ratchet (ADR-0008). Cache reads cost 0.05×, so stable prefixes matter — always-on rules stay few, short, and byte-stable (no dates or counters). `disable-model-invocation: true` drops a user-only ritual from that roster; `user-invocable: false` hides a reference-only skill from the menu while other skills can still load it.
