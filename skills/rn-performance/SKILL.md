---
name: rn-performance
description: React Native / Expo performance, build, and upgrade depth — FPS and re-render fixes, Hermes, TTI / startup, bundle and app size, FlashList, memory leaks, Reanimated UI-thread animation, Turbo Modules, Android 16KB page alignment, and React Native version upgrades. Use when debugging jank or frame drops, slow startup, large bundles, memory growth, bridge overhead, or when upgrading the RN/Expo version. For RN screen layout/UX polish use enhance-rn-screen; for emulator runs use start-emulator / test-emulator.
---

# React Native Performance & Build

> The performance, bundling, and upgrade layer for React Native / Expo. `enhance-rn-screen` handles screen layout and native feel; this skill handles *speed, size, and version moves*.
>
...
- After upgrade: clean caches (`watchman watch-del`, Metro cache, Pods, Gradle), rebuild, and smoke-test on both platforms via `test-emulator`.
...
## Composes with
- `enhance-rn-screen` — layout, safe-area, touch-target, native feel.
- `start-emulator` / `test-emulator` — boot + on-device verification.
