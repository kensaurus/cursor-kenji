...
# start-emulator — Metro + Android emulator in a sane order
...
> Pair with **`test-emulator`** for full QA; this skill focuses on **bring-up + terminal hygiene**.
...
**Heuristic:** if the user says *“stale bundle / nothing changed / white screen after hours of dev”* → lean **fresh** once, confirm **Phase 1.5-style bundle checks** from `test-emulator`. If they say *“just start my session”* → prefer **fast**.
...
Expect **`Physical size: 1080x2400`** for the default tap-QA preset (or `1080x4000` if scroll QA was requested). If you see `720×1280` or another unintended size, the AVD `skin.path` is overriding `hw.lcd.*` — use `adb shell wm size 1080x2400` to override at runtime, or apply the minimal custom skin workflow in **`test-emulator` Phase 0.5**.

### D.3 `_no_skin` failure

If the emulator exits immediately with **`unknown skin name '_no_skin'`**, remove/replace **`skin.name=_no_skin`** in the AVD **`config.ini`** — see **`test-emulator`** Phase **0.5b–c**.
...
| Emulator never in `adb devices` | AVD skin crash / HW issue | Check emulator stdout; **`test-emulator` 0.5** |
| `Physical size` wrong | Stock skin clobbering `hw.lcd` | `adb shell wm size 1080x2400` overrides at runtime; persistent fix → **`test-emulator` 0.5** |
...
- **`test-emulator`** — AVD **`layout`** template, guest/sign-in walks, DB/Sentry loops.
