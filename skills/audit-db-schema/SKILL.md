...
description: Audit database schema for consistency, robustness, and industry standards. Auto-detects database type (Supabase/Postgres/MySQL), ORM (Prisma/Drizzle/Sequelize), and migration tool. Live schema inspection via Supabase MCP, ORM docs via Context7. Covers naming conventions, data types, constraints, indexes, RLS policies, relationships, migrations, security. Use for "schema review", "naming conventions", "RLS audit", "migration check", "index audit", "constraint check", or data integrity concerns.
---
